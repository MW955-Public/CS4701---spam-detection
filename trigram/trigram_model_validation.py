...
ham_validation_list = Trigram.processed_file("../ham/validation.txt")
spam_validation_list = Trigram.processed_file("../spam/validation.txt")
...
with open('eval_k_0.1_1.0.csv', mode='w') as report_file:
    ...
    for k in np.linspace(0.1, 1.0, 10):
        ham_pro_dict = Trigram.trigram_probability_dict("../ham/train.txt", k)
        spam_pro_dict = Trigram.trigram_probability_dict("../spam/train.txt", k)

        ham_count_ham = ham_count_spam = 0
        for message in ham_validation_list:
            spam_pp = Trigram.perplexity_single_message(spam_pro_dict, message)
            ham_pp = Trigram.perplexity_single_message(ham_pro_dict, message)
            result = None
            if spam_pp > ham_pp:
                ham_count_ham += 1
            else:
